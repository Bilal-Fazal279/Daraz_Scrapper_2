...
const crawler = new BasicCrawler({
    ...
    // 💡 Everything utilizing "request" MUST live inside this handler function block
    async requestHandler({ request }) {
        const page = request.userData?.page || 1;
        const retryCount = request.retryCount || 0;

        // 🐢 Deep-page dynamic stealth backoff rules
        let baseDelay = 3000 + Math.random() * 3000; 
        if (page > 40) {
            console.log(`🐢 Deep page detected (${page}). Applying extra stealth delay rules...`);
            baseDelay = 8000 + Math.random() * 7000; // 8 to 15-second gap per page
        ...
        try {
            // Now request.url is perfectly scoped and defined here!
            const response = await gotScraping({
                url: request.url,
                headerGeneratorOptions: {
                    browsers: [
            ...
            /**
             * 3. HIGH-PERFORMANCE RECONCILIATION LAYER
             */
            try {
                const scrapedIds = products.map(item => String(item.itemId));
